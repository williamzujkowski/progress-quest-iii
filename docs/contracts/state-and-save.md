...
- Market arrival schedules `bulkStacks(equip) + nimbleStacks(stats)` ordered inventory stacks per one-second selling task, which is one at the fixture state and up to seven with a rated `Brassairts` and enough DEX. Ordinary stacks pay `floor(quantity * level * marketFavour(equip) * hagglingFavour(stats))`; names containing ` of ` apply the two canonical `RandomLow` multipliers before that scaling. Both scalings are applied after the draws rather than instead of one, so they cannot move the RNG stream, and both are inert at the fixture state — which is why the recorded goldens agree with the older, simpler sentence this replaces. Gold and sale events report only the amount actually credited at the persistence ceiling. Market exit buys for five seconds only when Gold is strictly greater than the level price; equality takes the four-second route back to the killing fields.
